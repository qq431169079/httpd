...
/* this fun for allow port */
/* the return ok means next  */
/* other means FORBIDDEN*/
static int proxy_flashapp_connect(request_rec *r)
{
	int status=0;
	const char *str=NULL;
	str=apr_table_get(r->headers_in, "X-Flashapp-Connect");
	if(str!=NULL){
		apr_table_unset(r->headers_in, "X-Flashapp-Connect");
        return OK;
	}
	return DECLINED;
}
