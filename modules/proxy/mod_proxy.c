...
/* this fun for invoke handler */
/* the return ok means next  */
/* other means goto  cleanup*/
static int proxy_flashapp_handler(request_rec *r, proxy_worker *worker,proxy_server_conf *conf,char *url)
{
	int status=0;
	int fp_port=80;
	char *fp_host=NULL;
	char *fp_p=NULL,*fp_q=NULL;
	const char *str=NULL;
	str=apr_table_get(r->headers_in, "X-Flashapp-Proxy");
	if(str!=NULL){
		fp_p=apr_pstrdup(r->pool,str);
		fp_q = strchr(fp_p, ':');
		if(fp_q!=NULL){
			if(sscanf(fp_q + 1, "%u", &fp_port) != 1|| fp_port > 65535){
				return DECLINED;
			}
			*fp_q = '\0';
		}
		fp_host=apr_pstrdup(r->pool,fp_p);
		ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,
		                      "proxy_flashapp_handler host:%s , port: %d ",fp_host,fp_port);
		apr_table_unset(r->headers_in, "X-Flashapp-Proxy");
		status=proxy_run_scheme_handler(r, worker,conf, url,fp_host,fp_port);
...
static int proxy_handler(request_rec *r)
{
    ...
    do {
        ...
        if (!direct_connect) {
        	//flashapp proxy
        	if(OK == proxy_flashapp_handler(r,worker,conf,url)){
        		goto cleanup;
        	}
        	//////////////////////////
